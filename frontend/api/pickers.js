...
export default new class extends BaseApi {
    ...
    changePick (picker, pick) {
        return this.axios.put(this.path(picker.uuid, 'picks', pick.uuid), {gameId: pick.game.id});
    }

    addComment (picker, comment) {
        return this.axios.post(this.path(picker.uuid, 'comments'), {text: comment.text});
    }
}
